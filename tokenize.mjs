...
const errors = {
  CommentIdentifierMustBeFollowedByASingleSpace: ({ char, pos, line }) =>
    'Comment identifier `~` must be followed by a single space ` `, ' + but_found(char, pos, line),
  CommentMayNotContainLeadingWhitespaces: ({ char, pos, line }) =>
    'Comment may not contain any leading whitespaces, ' + but_found(char, pos, line),
  CommentMayNotContainTrailingWhitespaces: ({ char, pos, line }) =>
    'Comment may not contain any trailing whitespaces, ' + but_found(char, pos, line),
  LineStartsWithUnknownIdentifier: ({ char, pos, line }) =>
    `Line starts with unknown identifier ${print_char(char)} (at position ${pos}, line ${line})`
}
